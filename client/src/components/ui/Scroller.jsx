...
export const Scroller = ({
    children,
    overflow = "y", // Default to 'y' if not provided
    height = "100%",
    width = "100%",
    withButtons = false,
                ...
                if (overflow === "y") {
                    if (direction === "next") {
                        if (rect && rect.top - 80 > 0) {
                            nearestIndex = i < items.length ? i + 1 : items.length;
                            break;
                        }
                    } else {
                        if (rect && rect.top - 80 > 0) {
    ...
    return (
        ...
        >
            {withButtons && overflow === "y" && (
                <div className="flex justify-center gap-2 m-[1px] z-10 w-full absolute top-2 right-2 pointer-events-none">
                    <div className="pointer-events-auto flex gap-2">
                        <Button
                            variant="secondary"
                            size="icon"
                            className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm"
            ...
            <div className={clsx(
                "absolute left-0 right-0 w-full h-10 bg-gradient-to-b from-black to-transparent pointer-events-none duration-300",
                showTopOverlay ? (withButtons ? "top-0" : "top-0") : "-top-10 opacity-0"
            )} />
            <div className={clsx(
                "absolute left-0 right-0 w-full h-10 bg-gradient-to-t from-black to-transparent pointer-events-none duration-300",
                showBottomOverlay ? "bottom-0" : "-bottom-10 opacity-0"
            )} />
            <div className={clsx(
                "absolute top-0 bottom-0 w-10 h-full bg-gradient-to-r from-black to-transparent pointer-events-none duration-300",
                showLeftOverlay ? "left-0" : "-left-10 opacity-0"
            )} />
            <div className={clsx(
                "absolute top-0 bottom-0 w-10 h-full bg-gradient-to-l from-black to-transparent pointer-events-none duration-300",
                showRightOverlay ? "right-0" : "-right-10 opacity-0"
            )} />
        </div>
    );
};
